...
void RARunnable::run()
{
    ...
    {
        sLog->outError("RA can not bind to port %d on %s", raport, stringip.c_str());
        return;
    }

    sLog->outString("RA on port %d on %s", raport, stringip.c_str());
    ...
    sLog->outStaticDebug("RA thread exiting");
}
