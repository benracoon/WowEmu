...
bool AuthSocket::_HandleLogonProof()
{
    ...
    if (_expversion == CL_EXPANSION_FLAG)
    {
    ...
    if (!memcmp(M.AsByteArray(), lp.M1, 20))
    {
        ...
        if (_expversion & LK_EXPANSION_FLAG)                 // 2.x and 3.x clients
        {
...
bool AuthSocket::_HandleRealmList()
{
    ...
    for (RealmList::RealmMap::const_iterator i = sRealmList->begin(); i != sRealmList->end(); ++i)
    {
        ...
        if ((_expversion & LK_EXPANSION_FLAG) && i->second.gamebuild != _build)
            continue;
        else if ((_expversion & BC_EXPANSION_FLAG) && !AuthHelper::GetAcceptedClientBuilds(i->second.gamebuild))
                continue;
        ...
        if ( _expversion & LK_EXPANSION_FLAG )               // only 2.x and 3.x clients
