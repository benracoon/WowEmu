...
RealmDBWorkerPool RealmDB;                      // Accessor to the RealmServer database
...
extern int main(int argc, char **argv)
{
    ...
    {
        sLog->outError("RealmServer can not bind to %s:%d", bind_ip.c_str(), rmport);
        return 1;
    }
    ...
    // Register RealmServer signal handlers
...
#ifdef _WIN32
    {
        ...
        if (Prio)
        {
            if (SetPriorityClass(hProcess, HIGH_PRIORITY_CLASS))
                sLog->outString("The RealmServer process priority class has been set to HIGH");
            else
                sLog->outError("Can't set RealmServer process priority class.");
            sLog->outString();
        }
    }
#endif
...
bool StartDB()
{
    ...
    {
        sLog->outError("Improper value specified for RealmDB.WorkerThreads, defaulting to 1.");
        worker_threads = 1;
    }
    ...
    {
        sLog->outError("Improper value specified for RealmDB.SynchThreads, defaulting to 1.");
        synch_threads = 1;
    }

    // NOTE: While RealmServer is singlethreaded you should keep synch_threads == 1. Increasing it is just silly since only 1 will be used ever.
