...
public:
    ...
    struct boss_aranAI : public ScriptedAI
    {
        ...
        {
            SpellEffectEntry const* effect0 = Spell->GetSpellEffect(EFFECT_0);
            SpellEffectEntry const* effect1 = Spell->GetSpellEffect(EFFECT_1);
            SpellEffectEntry const* effect2 = Spell->GetSpellEffect(EFFECT_2);
            //We only care about interrupt effects and only if they are durring a spell currently being casted
            if ((effect0->Effect != SPELL_EFFECT_INTERRUPT_CAST &&
                effect1->Effect != SPELL_EFFECT_INTERRUPT_CAST &&
                effect2->Effect != SPELL_EFFECT_INTERRUPT_CAST) || !me->IsNonMeleeSpellCasted(false))
                return;
