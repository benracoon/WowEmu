...
public:
    ...
    static bool HandleGameObjectTargetCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (*args)
        {
            ...
            if (id)
                result = WorldDB.PQuery("SELECT guid, id, position_x, position_y, position_z, orientation, map, phaseMask, (POW(position_x - '%f', 2) + POW(position_y - '%f', 2) + POW(position_z - '%f', 2)) AS order_ FROM gameobject WHERE map = '%i' AND id = '%u' ORDER BY order_ ASC LIMIT 1",
                pl->GetPositionX(), pl->GetPositionY(), pl->GetPositionZ(), pl->GetMapId(), id);
            else
            {
                std::string name = cId;
                WorldDB.EscapeString(name);
                result = WorldDB.PQuery(
        ...
        else
        {
            ...
            result = WorldDB.PQuery("SELECT gameobject.guid, id, position_x, position_y, position_z, orientation, map, phaseMask, "
    ...
    static bool HandleGameObjectNearCommand(ChatHandler* handler, const char* args)
    {
        ...
        Player* pl = handler->GetSession()->GetPlayer();
        QueryResult result = WorldDB.PQuery("SELECT guid, id, position_x, position_y, position_z, map, "
