...
void World::LoadConfigSettings(bool reload)
{
    ...
    SetMotd(sConfig->GetStringDefault("Motd", "Welcome to a Strawberry-Pr0jcts Server."));
...
void World::SetInitialWorldSettings()
{
    ...
    PreparedStatement* stmt = CharDB.GetPreparedStatement(CHAR_DEL_OLD_CORPSES);
    ...
    sLog->outString("Loading SpellEffectInfo store...");
    sSpellMgr->LoadSpellEffectInfoStore();
...
{
    PreparedStatement* stmt = CharDB.GetPreparedStatement(CHAR_GET_CHARACTER_COUNT);
    stmt->setUInt32(0, accountId);
    PreparedQueryResultFuture result = CharDB.AsyncQuery(stmt);
