...
void WorldSession::HandleSendMail(WorldPacket & recv_data)
{
    ...
    {
        SendNotification(GetStrings(LANG_MAIL_SENDER_REQ), sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
        return;
    }
    ...
    {
        SendNotification(GetStrings(LANG_MAIL_RECEIVER_REQ), sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
        return;
    }
            ...
            if (GetSecurity() > SEC_PLAYER && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
            {
                ...
                else
                {
                    ...
                    if (!sObjectMgr->GetPlayerNameByGUID(sender_guid, sender_name))
                        sender_name = sObjectMgr->GetStringsForDBCLocale(LANG_UNKNOWN);
                }
