...
void WorldSession::HandleAddFriendOpcode(WorldPacket & recv_data)
{
    ...
    std::string friendName = GetStrings(LANG_FRIEND_IGNORE_UNKNOWN);
    ...
    CharDB.EscapeString(friendName);            // prevent SQL injection - normal name must not be changed by this call
    ...
    _addFriendCallback.SetFutureResult(
        CharDB.AsyncPQuery("SELECT guid, race, account FROM characters WHERE name = '%s'", friendName.c_str())
        );
}
...
void WorldSession::HandleAddIgnoreOpcode(WorldPacket & recv_data)
{
    ...
    std::string IgnoreName = GetStrings(LANG_FRIEND_IGNORE_UNKNOWN);
    ...
    CharDB.EscapeString(IgnoreName);            // prevent SQL injection - normal name must not be changed by this call

    sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: %s asked to Ignore: '%s'",
        GetPlayer()->GetName(), IgnoreName.c_str());

    _addIgnoreCallback = CharDB.AsyncPQuery("SELECT guid FROM characters WHERE name = '%s'", IgnoreName.c_str());
}
...
void WorldSession::HandleBugOpcode(WorldPacket & recv_data)
{
    ...
    CharDB.EscapeString(type);
    CharDB.EscapeString(content);
    CharDB.PExecute ("INSERT INTO bugreport (type, content) VALUES('%s', '%s')", type.c_str(), content.c_str());
}
...
void WorldSession::HandleInspectHonorStatsOpcode(WorldPacket& recv_data)
{
    ...
    WorldPacket data(MSG_INSPECT_HONOR_STATS, 4 + 1 + 4 + 8);
...
void WorldSession::HandleWhoisOpcode(WorldPacket& recv_data)
{
    ...
    QueryResult result = RealmDB.PQuery("SELECT username, email, last_ip FROM account WHERE id=%u", accid);
    if (!result)
    {
