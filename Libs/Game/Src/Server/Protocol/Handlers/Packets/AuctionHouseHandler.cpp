...
void WorldSession::SendAuctionHello(uint64 guid, Creature* unit)
{
    ...
    {
        SendNotification(GetString(LANG_AUCTION_REQ), sWorld->getIntConfig(CONFIG_AUCTION_LEVEL_REQ));
        return;
    }
...
{
    WorldPacket data(SMSG_AUCTION_OWNER_NOTIFICATION, 4+8+4+8+4+4+4);
    data << auction->Id;
    data << auction->bid;
    data << uint32(0);                                     // unk
    data << uint64(0);                                     // unk
...
void WorldSession::HandleAuctionListPendingSales(WorldPacket & recv_data)
{
    ...
    if (!pCreature)
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleAuctionListPendingSales - Unit (GUID: %u) not found or you can't interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
