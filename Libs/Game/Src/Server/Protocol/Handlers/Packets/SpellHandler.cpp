...
void WorldSession::HandleUseItemOpcode(WorldPacket& recvPacket)
{
    ...
    uint8 slotIndex, slot, castFlags;
    uint8 castCount;                                       // next cast if exists (single or not)
    uint64 itemGUID;
    uint32 spellId;                                         // casted spell id

    recvPacket >> slotIndex >> slot >> castCount >> spellId >> itemGUID >> castFlags;

    Item *pItem = pUser->GetUseableItemByPos(slotIndex, slot);
    if (!pItem)
    {
    ...
    sLog->outDetail("WORLD: CMSG_USE_ITEM packet, slotIndex: %u, slot: %u, castCount: %u, spellId: %u, Item: %, data length = %i", slotIndex, slot, castCount, spellId, pItem->GetEntry(), (uint32)recvPacket.size());
...
void WorldSession::HandleOpenItemOpcode(WorldPacket& recvPacket)
{
    ...
    uint8 slotIndex, slot;

    recvPacket >> slotIndex >> slot;

    sLog->outDetail("bagIndex: %u, slot: %u",slotIndex, slot);

    Item *pItem = pUser->GetItemByPos(slotIndex, slot);
    if (!pItem)
    {
        pUser->SendEquipError(EQUIP_ERR_ITEM_NOT_FOUND, NULL, NULL);
        return;
    ...
    {
        QueryResult result = CharDB.PQuery("SELECT entry, flags FROM character_gifts WHERE item_guid = '%u'", pItem->GetGUIDLow());
        if (result)
        {
...
void WorldSession::HandleTotemDestroyed(WorldPacket& recvPacket)
{
    ...
    if (!_player->m_SummonSlot[slot])
        return;

    Creature* totem = GetPlayer()->GetMap()->GetCreature(_player->m_SummonSlot[slot]);
