...
void WorldSession::HandleArenaTeamCreate(WorldPacket & recv_data)
{
    ...
    if (!at->Create(uint32(_player->GetGUID()), type, name, background, icon, iconcolor, border, bordercolor))
    {
...
{
    WorldPacket data(SMSG_MULTIPLE_PACKETS, 2+4+team.length()+1+player.length()+1+4);
    data << uint16(SMSG_ARENA_TEAM_COMMAND_RESULT);
