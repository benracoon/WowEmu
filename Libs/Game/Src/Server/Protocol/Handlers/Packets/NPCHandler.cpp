...
{
    std::string str = GetString(LANG_NPC_TAINER_HELLO);
...
void WorldSession::SendStablePet(uint64 guid)
{
    ...
    m_sendStabledPetCallback.SetFutureResult(
        CharDB.AsyncPQuery("SELECT owner, slot, id, entry, level, name FROM character_pet WHERE owner = '%u' AND slot >= '%u' AND slot <= '%u' ORDER BY slot",
...
void WorldSession::HandleStableChangeSlot(WorldPacket & recv_data)
{
    ...
    m_stableChangeSlotCallback.SetFutureResult(
                                               CharDB.PQuery("SELECT slot,entry,id FROM character_pet WHERE owner = '%u' AND id = '%u'",
...
void WorldSession::HandleStableChangeSlotCallback(QueryResult result, uint8 new_slot)
{
    ...
    CharDB.PExecute("UPDATE character_pet SET slot = '%u' WHERE slot = '%u' AND owner='%u'",                      new_slot, slot, GetPlayer()->GetGUIDLow());
    CharDB.PExecute("UPDATE character_pet SET slot = '%u' WHERE slot = '%u' AND owner='%u' AND id<>'%u'", slot, new_slot, GetPlayer()->GetGUIDLow(), pet_number);
