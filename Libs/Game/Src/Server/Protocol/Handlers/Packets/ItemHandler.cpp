...
void WorldSession::HandleListInventoryOpcode(WorldPacket & recv_data)
{
    ...
    SendVendorInventory(guid);
}

void WorldSession::SendVendorInventory(uint64 vendorguid)
{
    ...
    if (!pCreature)
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: SendVendorInventory - Unit (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(vendorguid)));
        _player->SendSellError(SELL_ERR_CANT_FIND_VENDOR, NULL, 0, 0);
        return;
    }
    ...
    if (!vItems)
    {
        WorldPacket data(SMSG_VENDOR_INVENTORY, (3 + 4 + 4));
        data << uint8(0);
        data << uint16(vendorguid);
        data << uint32(0);                                   // count == 0, next will be error code
        data << uint32(0);                                   // "Vendor has no inventory"
        SendPacket(&data);
        return;
    }
    ...
    WorldPacket data(SMSG_VENDOR_INVENTORY, (3 + 4 + 1 + numitems * 9 * 4 + 4 * numitems ));
    
    //data << uint64(vendorguid);    // ToDo: Update to new bitstream...
