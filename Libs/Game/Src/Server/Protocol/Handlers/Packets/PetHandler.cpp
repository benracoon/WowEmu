...
void WorldSession::HandlePetActionHelper(Unit *pet, uint64 guid1, uint16 spellid, uint16 flag, uint64 guid2)
{
    ...
    switch(flag)
    {
        ...
        case ACT_ENABLED:                                   // 0xC1    spell
        {
            ...
            {
                SpellEffectEntry const* iSpellEffect = spellInfo->GetSpellEffect(SpellEffIndex(i));
                if (!iSpellEffect)
                    continue;

                if (iSpellEffect->EffectImplicitTargetA == TARGET_UNIT_AREA_ENEMY_SRC || iSpellEffect->EffectImplicitTargetA == TARGET_UNIT_AREA_ENEMY_DST || iSpellEffect->EffectImplicitTargetA == TARGET_DEST_DYNOBJ_ENEMY)
                    return;
            }
...
void WorldSession::HandlePetAbandon(WorldPacket & recv_data)
{
    ...
    if (pet)
    {
        if (pet->isPet())
        {
            ...
            {
                uint32 feelty = pet->GetPower(HAPPINESS);
                pet->SetPower(HAPPINESS ,(feelty-50000) > 0 ?(feelty-50000) : 0);
            }
