...
void WorldSession::HandleGMTicketCreateOpcode(WorldPacket & recv_data)
{
    ...
    {
        SendNotification(GetStrings(LANG_TICKET_REQ), sWorld->getIntConfig(CONFIG_TICKET_LEVEL_REQ));
        return;
    }
...
void WorldSession::HandleGMSurveySubmit(WorldPacket& recv_data)
{
    ...
    for (uint8 i = 0; i < 10; i++)
    {
        ...
        PreparedStatement* stmt = CharDB.GetPreparedStatement(CHAR_ADD_GM_SUBSURVEY);
    ...
    PreparedStatement* stmt = CharDB.GetPreparedStatement(CHAR_ADD_GM_SURVEY);
...
void WorldSession::HandleReportLag(WorldPacket& recv_data)
{
    ...
    PreparedStatement* stmt = CharDB.GetPreparedStatement(CHAR_ADD_LAG_REPORT);
    stmt->setUInt32(0, GUID_LOPART(GetPlayer()->GetGUID()));
