...
{
    char const *format = GetStrings(string_id);
    if (format)
    {
        va_list ap;
        char szStr[1024];
...
const char *WorldSession::GetStrings(int32 entry) const
{
...
void WorldSession::SetAccountData(AccountDataType type, time_t tm, std::string data)
{
    ...
    PreparedStatement* stmt = CharDB.GetPreparedStatement(index);
    stmt->setUInt32(0, id);
...
void WorldSession::LoadTutorialsData()
{
    ...
    PreparedStatement* stmt = CharDB.GetPreparedStatement(CHAR_LOAD_TUTORIALS);
    stmt->setUInt32(0, GetAccountId());
...
void WorldSession::SaveTutorialsData(SQLTransaction &trans)
{
    ...
    PreparedStatement* stmt = CharDB.GetPreparedStatement(CHAR_GET_HAS_TUTORIALS);
    stmt->setUInt32(0, GetAccountId());
    bool hasTutorials = !CharDB.Query(stmt).null();
    // Modify data in DB
    stmt = CharDB.GetPreparedStatement(hasTutorials ? CHAR_SET_TUTORIALS : CHAR_ADD_TUTORIALS);
    for (uint8 i = 0; i < MAX_ACCOUNT_TUTORIAL_VALUES; ++i)
