...
{
    PathFinder path(this, x, y, z, !smoothPath, forceDest);
...
{
    uint32 flags = GetTypeId() == TYPEID_PLAYER ? MOVEMENTFLAG_WALKING : ((Creature*)this)->GetUnitMovementFlags();
...
void Unit::SendMonsterMoveByPath(Path<Elem,Node> const& path, uint32 start, uint32 end, uint32 flags, uint32 traveltime)
{
    uint32 pathSize = end - start;

    if (pathSize < 1)
    {
        SendMonsterMove(GetPositionX(), GetPositionY(), GetPositionZ(), uint32(SPLINETYPE_STOP), flags, 0);
        return;
    }

    if (pathSize == 1)
    {
        SendMonsterMove(path[start].x, path[start].y, path[start].z, uint32(SPLINETYPE_NORMAL), flags, traveltime);
        return;
    }

    uint32 packSize = (flags & uint32(SPLINEFLAG_FLYING | SPLINEFLAG_CATMULLROM)) ? pathSize*4*3 : 4*3 + (pathSize-1)*4;
    ...
    if (flags & uint32(SPLINEFLAG_FLYING | SPLINEFLAG_CATMULLROM))
    {
...
template void Unit::SendMonsterMoveByPath<PathNode>(const Path<PathNode> &, uint32, uint32, uint32, uint32);
template void Unit::SendMonsterMoveByPath<TaxiPathNodePtr, const TaxiPathNodeEntry>(const Path<TaxiPathNodePtr, const TaxiPathNodeEntry> &, uint32, uint32, uint32, uint32);
...
void Unit::SendMonsterMove(float NewPosX, float NewPosY, float NewPosZ, uint32 MoveFlags, uint32 time, float speedZ, Player *player)
{
    ...
    if (MoveFlags & SPLINEFLAG_FINALFACING)
    {
        data << uint8(0);
        data << uint32(0);
    }
...
bool Unit::HasAuraTypeWithFamilyFlags(AuraType auraType, uint32 familyName, uint32 familyFlags) const
{
    ...
    for (AuraEffectList::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
    {
        ...
        {
            SpellClassOptionsEntry const* classOptions = iterSpellProto->GetSpellClassOptions();
            if (iterSpellProto->GetSpellFamilyName() == familyName && classOptions->SpellFamilyFlags & familyFlags)
                return true;
        }
    }
    return false;
...
uint32 Unit::DealDamage(Unit *pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const *spellProto, bool durabilityLoss)
{
    ...
    if (cleanDamage && damagetype == DIRECT_DAMAGE && this != pVictim && getPowerType() == RAGE)
    {
    ...
    if (!damage)
    {
        ...
        if (cleanDamage && cleanDamage->absorbed_damage && pVictim->getPowerType() == RAGE)
    ...
    else
    {
        ...
        if (this != pVictim && pVictim->getPowerType() == RAGE)
        {
...
bool Unit::IsDamageReducedByArmor(SpellSchoolMask schoolMask, SpellEntry const *spellInfo, uint8 effIndex)
{
    ...
    if (spellInfo)
    {
        ...
        SpellEffectEntry const* spellEffect = spellInfo->GetSpellEffect(SpellEffIndex(effIndex));
        if (!spellEffect)
            return false;

        // bleeding effects are not reduced by armor
        if (effIndex != MAX_SPELL_EFFECTS && (spellEffect->EffectApplyAuraName == SPELL_AURA_PERIODIC_DAMAGE))
...
void Unit::CalcAbsorbResist(Unit *pVictim, SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, uint32 *absorb, uint32 *resist, SpellEntry const *spellInfo)
{
    ...
    for (AuraEffectList::const_iterator itr = vManaShieldCopy.begin(); (itr != vManaShieldCopy.end()) && (dmgInfo.GetDamage() > 0); ++itr)
    {
        ...
        int32 manaTaken = -pVictim->ModifyPower(MANA, -manaReduction);
...
int32 Unit::GetMechanicResistChance(const SpellEntry *spell)
{
    ...
    {
        SpellEffectEntry const* spellEffect = spell->GetSpellEffect(SpellEffIndex(eff));
        if (!spellEffect)
            continue;
...
SpellMissInfo Unit::MagicSpellHitResult(Unit *pVictim, SpellEntry const *spell)
{
    ...
    if (!IsPositiveSpell(spell->Id))
    {
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            ...
            if (spellEffect->EffectApplyAuraName != 0)
            {
                bNegativeAura = true;
                break;
            }
...
bool Unit::CanCastWhileWalking(const SpellEntry * const sp)
{
    ...
    SpellClassOptionsEntry const* classOptions = sp->GetSpellClassOptions();
    if (!classOptions)
        return false;
    ...
    for (AuraEffectList::const_iterator i = alist.begin(); i != alist.end(); ++i)
    {
        ...
        if(!((*i)->GetSpellProto()->GetEffectSpellClassMask(SpellEffIndex((*i)->GetEffIndex())) && 
            classOptions->SpellFamilyFlags))
            continue;
        return true;
    }
    return false;
...
void Unit::_AddAura(UnitAura * aura, Unit * caster)
{
    ...
    if (!aura->IsPassive() && aura->GetId() != 44413)
    {
        ...
        if (Aura * foundAura = GetOwnedAura(aura->GetId(), aura->GetCasterGUID(), (sSpellMgr->GetSpellCustomAttr(aura->GetId()) & SPELL_ATTR_CU_ENCHANT_PROC) ? aura->GetCastItemGUID() : 0, 0, aura))
        {
...
bool Unit::_IsNoStackAuraDueToAura(Aura * appliedAura, Aura * existingAura) const
{
    ...
    {
        SpellEffectEntry const* spellEffect = i_spellProto->GetSpellEffect(SpellEffIndex(j));
        if (!spellEffect)
            continue;

        if (spellEffect->EffectTriggerSpell == spellProto->Id
            || spellEffect->EffectTriggerSpell == i_spellProto->Id) // I do not know what is this for
        {
...
void Unit::RemoveAurasDueToSpellByDispel(uint32 spellId, uint64 casterGUID, Unit *dispeller)
{
    for (AuraMap::iterator iter = m_ownedAuras.lower_bound(spellId); iter != m_ownedAuras.upper_bound(spellId);)
    {
        ...
        if (aura->GetCasterGUID() == casterGUID)
        {
            ...
            SpellClassOptionsEntry const* classOptions = aura->GetSpellProto()->GetSpellClassOptions();

            // Unstable Affliction (crash if before removeaura?)
            if (aura->GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_WARLOCK && (classOptions->SpellFamilyFlags & 0x0100))
            {
            ...
            // Flame Shock
            if (aura->GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_SHAMAN && (classOptions->SpellFamilyFlags & 0x10000000))
            {
                Unit * caster = aura->GetCaster();
                if (caster)
                {
...
void Unit::RemoveAurasWithFamily(SpellFamilyNames family, uint32 familyFlag1, uint32 familyFlag2, uint32 familyFlag3, uint64 casterGUID)
{
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end();)
    {
        ...
        if (!casterGUID || aura->GetCasterGUID() == casterGUID)
        {
            ...
            if (spell->GetSpellFamilyName() == uint32(family) && classOptions->SpellFamilyFlags && classOptions->SpellFamilyFlags2)
            {
                RemoveAura(iter);
                continue;
            }
...
AuraEffect* Unit::GetAuraEffect(AuraType type, SpellFamilyNames name, uint32 iconId, uint8 effIndex) const
{
    ...
    for (Unit::AuraEffectList::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
    {
        ...
        if (spell->SpellIconID == iconId && spell->GetSpellFamilyName() == uint32(name) && !classOptions->SpellFamilyFlags)
            return *itr;
    }
    return NULL;
}
...
bool Unit::HandleDummyAuraProc(Unit *pVictim, uint32 damage, AuraEffect* triggeredByAura, SpellEntry const * procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown)
{
    ...
    SpellClassOptionsEntry const* classOptions = dummySpell->GetSpellClassOptions();
    ...
    switch(dummySpell->GetSpellFamilyName())
    {
        ...
        case SPELLFAMILY_MAGE:
        {
            ...
            if (classOptions->SpellFamilyFlags & 0x8000)
            {
                if (GetTypeId() != TYPEID_PLAYER)
                    return false;
            ...
            switch(dummySpell->Id)
            {
                ...
                case 56372:
                {
                    ...
                    for (SpellCooldowns::const_iterator itr = SpellCDs.begin(); itr != SpellCDs.end(); itr++)
                    {
                        ...
                        SpellClassOptionsEntry const* classOpt = SpellCDs_entry->GetSpellClassOptions();
                        // Frost Nova
                        if (SpellCDs_entry && SpellCDs_entry->GetSpellFamilyName() == SPELLFAMILY_MAGE
                           && classOpt->SpellFamilyFlags & 0x00000040)
        ...
        case SPELLFAMILY_WARRIOR:
        {
            ...
            // Retaliation
            if (classOptions->SpellFamilyFlags & 0x8)
            {
        ...
        {
            // Seed of Corruption
            if (classOptions->SpellFamilyFlags & 0x00000010)
            {
            ...
            if (classOptions->SpellFamilyFlags == 0x0 && dummySpell->SpellIconID == 1932)
            {
        ...
        {
            // Vampiric Touch
            if (classOptions->SpellFamilyFlags & 0x00000400)
            {
                if (!pVictim || !pVictim->isAlive())
                    return false;
            ...
            switch (dummySpell->Id)
            {
                ...
                case 40438:
                {
                    ...
                    // Renew
                    else if (classOptions->SpellFamilyFlags & 0x40)
                ...
                case 55680:
                {
                    ...
                    int32 tickcount = GetSpellMaxDuration(GoPoH) / spellEff->EffectAmplitude;
                    if (!tickcount)
                        return false;
                ...
                case 70770:
                    // Flash Heal
                    if (classOptions->SpellFamilyFlags & 0x800)
                    {
                        ...
                        SpellEffectEntry const* spellEffect = blessHealing->GetSpellEffect(SpellEffIndex(triggered_spell_id));
                        if (!spellEffect)
                            return false;

                        basepoints0 = int32(CalculatePctN(damage, triggerAmount) / (GetSpellMaxDuration(blessHealing) / spellEffect->EffectAmplitude));
                    }
                    break;
            }
            break;
        ...
        case SPELLFAMILY_DRUID:
        {
            switch(dummySpell->Id)
            {
                ...
                case 54821:
                {
                    SpellEffectEntry const* spellEffect = procSpell->GetSpellEffect(SpellEffIndex(EFFECT_1));
                    if (!spellEffect)
                        return false;

                    if (procSpell->SpellVisual[0] == 750 && spellEffect->EffectApplyAuraName == 3)
                    {
                ...
                case 40442:
                {
                    ...
                    // Rejuvenation
                    else if (classOptions->SpellFamilyFlags & 0x10)
                    {
                        triggered_spell_id = 40446;
                        chance = 25.0f;
                    }
                    // Mangle (Bear) and Mangle (Cat)
                    else if (classOptions->SpellFamilyFlags & 0x00000440)
                    {
                ...
                {
                    // Wrath & Starfire
                    if ((classOptions->SpellFamilyFlags & 0x5) && (procEx & PROC_EX_CRITICAL_HIT))
                    {
                        ...
                        SpellEffectEntry const* spellEffect = triggeredSpell->GetSpellEffect(SpellEffIndex(EFFECT_0));
                        if (!spellEffect)
                            return false;

                        basepoints0 = CalculatePctN(int32(damage), triggerAmount) / (GetSpellMaxDuration(triggeredSpell) / spellEffect->EffectAmplitude);
                    }
                    break;
                }
            ...
            if (dummySpell->SpellIconID == 2856 && GetTypeId() == TYPEID_PLAYER)
            {
                ...
                bool isWrathSpell = (classOptions->SpellFamilyFlags & 1);
            ...
            else if (dummySpell->SpellIconID == 2850)
            {
                ...
                if (effIndex == 0)
                {
                    if (!(classOptions->SpellFamilyFlags & 0x00080000))
                        return false;
                ...
                {
                    if (!(classOptions->SpellFamilyFlags2 & 0x00000800))
                        return false;
        ...
        case SPELLFAMILY_HUNTER:
        {
            ...
            if (dummySpell->SpellIconID == 2236)
            {
                ...
                // Explosive Shot
                if (classOptions->SpellFamilyFlags2 & 0x200)
                {
            ...
            if (dummySpell->SpellIconID == 3560)
            {
                ...
                if (!(classOptions->SpellFamilyFlags & 0x01000000))
                    return false;
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (classOptions->SpellFamilyFlags & 0x8000000)
            {
                if (effIndex != 0)
                    return false;
            ...
            if (classOptions->SpellFamilyFlags & 0x00080000)
            {
                if (procFlag & PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_POS)
                {
                    if (procSpell->GetSpellFamilyName() == SPELLFAMILY_PALADIN
                        && (classOptions->SpellFamilyFlags & 0x40000000))
                    {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 20186:
                {
                    if (pVictim && pVictim->isAlive() && pVictim->getPowerType() == MANA)
                    {
                        // 2% of base mana
                ...
                case 40470:
                {
                    ...
                    if (classOptions->SpellFamilyFlags & 0xC0000000)
                    {
                ...
                case 54939:
                {
                    ...
                    for (uint8 i = 0; i <MAX_SPELL_EFFECTS; i++)
                    {
                        SpellEffectEntry const* spellEffect = procSpell->GetSpellEffect(SpellEffIndex(i));
                        if (!spellEffect)
                            continue;
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch(dummySpell->Id)
            {
                ...
                case 40463:
                {
                    ...
                    float chance;
                    if (classOptions->SpellFamilyFlags & 0x1)
                    {
                        triggered_spell_id = 40465;         // Lightning Bolt
                        chance = 15.0f;
                    }
                    else if (classOptions->SpellFamilyFlags & 0x80)
                    {
                        triggered_spell_id = 40465;         // Lesser Healing Wave
                        chance = 10.0f;
                    }
                    else if (classOptions->SpellFamilyFlags & 0x00000010)
                    {
                ...
                {
                    // Chain Heal
                    if((classOptions->SpellFamilyFlags & 0x100) && (procEx & PROC_EX_CRITICAL_HIT))
                    {
                ...
                case 70811:
                {
                    ...
                    if(classOptions->SpellFamilyFlags & 0x3)
                    {
                ...
                case 63280: // Glyph of Totem of Wrath
                {
                    ...
                    for (AuraEffectList::const_iterator i = auras.begin(); i != auras.end(); ++i)
                    {
                        ...
                        if (spell->GetSpellFamilyName() == uint32(SPELLFAMILY_SHAMAN) && classOpt->SpellFamilyFlags == 0x02000000)
                        {
            ...
            if (classOptions->SpellFamilyFlags & 0x200000)
            {
            ...
            if (dummySpell->SpellIconID == 2287)
            {
                ...
                if (classOptions->SpellFamilyFlags & 0x80)
                    // Lesser Healing Wave - 0.6 of default
                    chance *= 0.6f;
                else if (classOptions->SpellFamilyFlags & 0x100)
                ...
                {
                    SpellEffectEntry const* spellEffect = aurEff->GetSpellProto()->GetSpellEffect(SpellEffIndex(aurEff->GetEffIndex()));
                    if (!spellEffect)
                        return false;

                    uint32 spell = spellEffect->EffectTriggerSpell;
            ...
            if (dummySpell->SpellIconID == 2018)            // only this spell have SpellFamily Shaman SpellIconID == 2018 and dummy aura
            {
                ...
                {
                    SpellEffectEntry const* spellEffect = procSpell->GetSpellEffect(SpellEffIndex(effIndex));
                    if (!spellEffect)
                        return false;
                    ...
                    float chance = 100.0f / spellEffect->EffectChainTarget;
                    if (!roll_chance_f(chance))
                        return false;
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            if (dummySpell->SpellIconID == 138)
            {
                ...
                SpellEffectEntry const* spellEffect = dummySpell->GetSpellEffect(SpellEffIndex(effIndex));
                if (!spellEffect)
                    return false;

                triggered_spell_id = spellEffect->EffectTriggerSpell;
                break;
            }
            ...
            // Vendetta
            if (classOptions->SpellFamilyFlags & 0x10000)
            {
            ...
            if (dummySpell->SpellIconID == 1939 && GetTypeId() == TYPEID_PLAYER)
            {
                ...
                for (PlayerSpellMap::const_iterator itr = sp_list.begin(); itr != sp_list.end(); ++itr)
                {
                    ...
                    if (spellProto->GetSpellFamilyName() == SPELLFAMILY_DEATHKNIGHT
                        && classOptions->SpellFamilyFlags & 0x2000)
                    {
            ...
            if (dummySpell->Id == 17619)
            {
                if (procSpell->GetSpellFamilyName() == SPELLFAMILY_POTION)
                {
                    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; i++)
                    {
                        ...
                        else if (spellEffect->Effect == SPELL_EFFECT_ENERGIZE)
                        {
...
bool Unit::HandleObsModEnergyAuraProc(Unit *pVictim, uint32 /*damage*/, AuraEffect* triggeredByAura, SpellEntry const * /*procSpell*/, uint32 /*procFlag*/, uint32 /*procEx*/, uint32 cooldown)
{
    ...
    switch(dummySpell->GetSpellFamilyName())
    {
        ...
        {
            // Aspect of the Viper
            if (classOptions->SpellFamilyFlags & 0x40000)
            {
...
bool Unit::HandleModDamagePctTakenAuraProc(Unit *pVictim, uint32 /*damage*/, AuraEffect* triggeredByAura, SpellEntry const * /*procSpell*/, uint32 /*procFlag*/, uint32 /*procEx*/, uint32 cooldown)
{
    ...
    switch(dummySpell->GetSpellFamilyName())
    {
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (classOptions->SpellFamilyFlags & 0x10000000)
            {
                switch (getPowerType())
                {
    ...
    switch(dummySpell->GetSpellFamilyName())
    {
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (dummySpell->SpellIconID == 3021)
            {
                ...
                if (classOptions->SpellFamilyFlags & 0x40000000 && procSpell->SpellIconID == 242)
                {
                    *handled = true;
                ...
                else if ((classOptions->SpellFamilyFlags & 0x200000 || classOptions->SpellFamilyFlags & 0x10000) && !(procEx & PROC_EX_CRITICAL_HIT))
                    *handled = true;
                break;
            }
...
bool Unit::HandleProcTriggerSpell(Unit *pVictim, uint32 damage, AuraEffect* triggeredByAura, SpellEntry const *procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    SpellEffectEntry const* spellEffect = auraSpellInfo->GetSpellEffect(SpellEffIndex(triggeredByAura->GetEffIndex()));
    if (!spellEffect)
        return false;

    SpellClassOptionsEntry const* classOptions = auraSpellInfo->GetSpellClassOptions();

    // Basepoints of trigger aura
    int32 triggerAmount = triggeredByAura->GetAmount();

    // Set trigger spell id, target, custom basepoints
    uint32 trigger_spell_id = spellEffect->EffectTriggerSpell;
    ...
    if (sSpellStore.LookupEntry(trigger_spell_id) == NULL)
    {
        switch (auraSpellInfo->GetSpellFamilyName())
        {
            ...
            {
                // Drain Soul
                if (classOptions->SpellFamilyFlags & 0x4000)
                {
                    // Improved Drain Soul
            ...
            case SPELLFAMILY_HUNTER:
            {
                if (auraSpellInfo->SpellIconID == 3247)     // Piercing Shots
                {
                    ...
                    SpellEffectEntry const* spellEff0 = TriggerPS->GetSpellEffect(SpellEffIndex(EFFECT_0));
                    if (!spellEffect)
                        return false;

                    basepoints0 = CalculatePctN(int32(damage), triggerAmount) / (GetSpellMaxDuration(TriggerPS) / spellEff0->EffectAmplitude);
                ...
                switch (auraSpellInfo->Id)
                {
                    ...
                    default:
                        ...
                        if (auraSpellInfo->SpellIconID == 241)
                        {
                            ...
                            // Holy Shock heal
                            if (classOptions->SpellFamilyFlags & 0x00010000)
                            {
                                switch(procSpell->Id)
                                {
            ...
            case SPELLFAMILY_SHAMAN:
            {
                switch (auraSpellInfo->Id)
                {
                    ...
                    default:
                    {
                        SpellClassOptionsEntry const* classOpt = auraSpellInfo->GetSpellClassOptions();
                        // Lightning Shield (overwrite non existing triggered spell call in spell.dbc
                        if (classOpt->SpellFamilyFlags & 0x400)
                        {
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        {
            if (classOptions->SpellFamilyFlags & 0x800000)
    ...
    switch(trigger_spell_id)
    {
        ...
        case 35095:
        {
            if (!procSpell || procSpell->powerType != MANA || (procSpell->GetManaCost() == 0 && procSpell->GetManaCostPercentage() == 0 && procSpell->GetManaCostPerlevel() == 0))
                return false;
            break;
        }
        // Demonic Pact
        ...
        case 58679:
        {
            ...
            if (!procSpell || !(procSpell->GetSpellFamilyName() == SPELLFAMILY_DEATHKNIGHT && classOptions->SpellFamilyFlags == 0x80002000))
                return false;
            break;
        }
        // Glyph of Death Grip
        ...
        case 65081:
        {
            ...
            if (!(classOptions->SpellFamilyFlags & 0x00000001))
                return false;
            break;
        }
        // Culling the Herd
        case 70893:
        {
            ...
            if (!(classOptions->SpellFamilyFlags & 0x10000000) && (procEx != PROC_EX_CRITICAL_HIT) )
                return false;
...
void Unit::SetMinion(Minion *minion, bool apply, PetSlot slot)
{
    ...
    else
    {
        ...
        if (const SpellEntry* spInfo = sSpellStore.LookupEntry(minion->ToTotem()->GetSpell()))
            ...
            {
                SpellEffectEntry const* spellEffect = spInfo->GetSpellEffect(SpellEffIndex(i));
                if (!spellEffect)
                    continue;
                ...
                this->RemoveAllMinionsByEntry(spellEffect->EffectMiscValue);
            }
        }
...
uint32 Unit::SpellDamageBonus(Unit *pVictim, SpellEntry const *spellProto, uint32 effIndex, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (spellProto)
    {
        ...
        for (AuraEffectList::const_iterator i = mModDamagePercentDone.begin(); i != mModDamagePercentDone.end(); ++i)
            if ((*i)->GetMiscValue() & GetSpellSchoolMask(spellProto))
            {
                ...
                else if (!((*i)->GetSpellProto()->AttributesEx5 & SPELL_ATTR_EX5_SPECIAL_ITEM_CLASS_CHECK))
                {
                    SpellEquippedItemsEntry const* equippedItems = spellProto->GetSpellEquippedItems();
                    if ((*i)->GetSpellProto()->GetEquippedItemClass() & spellProto->GetEquippedItemClass())
                        if ((equippedItems->EquippedItemSubClassMask == 0) ||
                            (equippedItems->EquippedItemSubClassMask & equippedItems->EquippedItemSubClassMask))
                            AddPctN(DoneTotalMod, (*i)->GetAmount());
                }
        ...
        switch ((*i)->GetMiscValue())
        {
            ...
            case 4993:
            {
                ...
                for (AuraApplicationMap::const_iterator itr = victimAuras.begin(); itr != victimAuras.end(); ++itr)
                {
                    ...
                    SpellClassOptionsEntry const* classOptions = m_spell->GetSpellClassOptions();

                    if (m_spell->GetSpellFamilyName() != SPELLFAMILY_WARLOCK || !(classOptions->SpellFamilyFlags & 0x0004071B || classOptions->SpellFamilyFlags & 0x8044C402))
                        continue;
    ...
    switch(spellProto->GetSpellFamilyName())
    {
        case SPELLFAMILY_MAGE:
            ...
            // Torment the weak
            if (classOptions->SpellFamilyFlags & 0x20200021 || classOptions->SpellFamilyFlags & 0x9000)
        ...
        case SPELLFAMILY_PALADIN:
            ...
            if ((classOptions->SpellFamilyFlags & 0x400000) && spellProto->SpellIconID == 2292)
            {
        ...
        case SPELLFAMILY_WARLOCK:
            //Fire and Brimstone
            if (classOptions->SpellFamilyFlags & 0x00020040)
            ...
            if (classOptions->SpellFamilyFlags & 0x00004000)
                if (HasAura(200000))
                    DoneTotalMod *= 4;
        break;
        case SPELLFAMILY_DEATHKNIGHT:
            ...
            // Glacier Rot
            if (classOptions->SpellFamilyFlags & 0x2 || classOptions->SpellFamilyFlags & 0x6)
    ...
    if (HasUnitTypeMask(UNIT_MASK_GUARDIAN))
        if (!(GetSpellSchoolMask(spellProto) & SPELL_SCHOOL_MASK_NORMAL))                 // Do not add Spellpower to melee abilities
            ...
            DoneAdvertisedBenefit += int32(GetTotalAttackPowerValue(BASE_ATTACK) * 0.2f);
    ...
    if (DoneAdvertisedBenefit || TakenAdvertisedBenefit)
    {
        if(coeff <= 0)
            if(effIndex < 4)
                coeff = spellEffect->EffectBonusCoefficient;
            else
                coeff = 0;
...
int32 Unit::SpellBaseDamageBonus(SpellSchoolMask schoolMask)
{
    ...
    {
        SpellEquippedItemsEntry const* equipItem = (*i)->GetSpellProto()->GetSpellEquippedItems();
        if (((*i)->GetMiscValue() & schoolMask) != 0 &&
        (*i)->GetSpellProto()->GetEquippedItemClass() == -1 &&
                                                            // -1 == any item class (not wand then)
        equipItem->EquippedItemInventoryTypeMask == 0)
...
bool Unit::isSpellCrit(Unit *pVictim, SpellEntry const *spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType) const
{
    ...
    int32 damage = int32(crit_chance);
            ...
            if (pVictim)
            {
                ...
                switch (spellProto->GetSpellFamilyName())
                {
                    case SPELLFAMILY_DRUID:
                        ...
                        // Starfire
                        if (classOptions->SpellFamilyFlags & 0x4 && spellProto->SpellIconID == 1485)
                        {
                    ...
                    case SPELLFAMILY_PALADIN:
                        // Flash of light
                        if (classOptions->SpellFamilyFlags & 0x40000000)
                        {
                            // Sacred Shield
                    ...
                    case SPELLFAMILY_SHAMAN:
                        // Lava Burst
                        if (classOptions->SpellFamilyFlags & 0x00001000)
                        {
            ...
            if (pVictim)
            {
                ...
                switch(spellProto->GetSpellFamilyName())
                {
                    case SPELLFAMILY_DRUID:
                        ...
                        if (classOptions->SpellFamilyFlags & 0x00800000
                    ...
                    case SPELLFAMILY_PALADIN:
                        ...
                        if (spellProto->GetSpellFamilyName() == SPELLFAMILY_PALADIN)
                            if (classOptions->SpellFamilyFlags & 0x0000000000800000LL && spellProto->SpellIconID == 561)
...
uint32 Unit::SpellHealingBonus(Unit *pVictim, SpellEntry const *spellProto, uint32 effIndex, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
            ...
            case 8477: // Nourish Heal Boost
            {
                ...
                for (AuraApplicationMap::const_iterator itr = victimAuras.begin(); itr != victimAuras.end(); ++itr)
                {
                    ...
                    if (m_spell->GetSpellFamilyName() != SPELLFAMILY_DRUID ||
                        !(classOptions->SpellFamilyFlags & 0x00000010 || classOptions->SpellFamilyFlags & 0x50))
                        continue;
                    modPercent += stepPercent * aura->GetStackAmount();
                }
                AddPctN(DoneTotalMod, modPercent);
    ...
    else // scripted bonus
    {
        SpellClassOptionsEntry const* classOptions = spellProto->GetSpellClassOptions();
        // Gift of the Naaru
        if (classOptions->SpellFamilyFlags2 & 0x80000000 && spellProto->SpellIconID == 329)
        {
            scripted = true;
        ...
        else if (spellProto->GetSpellFamilyName() == SPELLFAMILY_SHAMAN && classOptions->SpellFamilyFlags & 0x80000)
            factorMod *= 0.45f;
    ...
    // Nourish cast
    if (spellProto->GetSpellFamilyName() == SPELLFAMILY_DRUID && classOptions->SpellFamilyFlags & 0x2000000)
    {
    ...
    // Healing Wave
    if (spellProto->GetSpellFamilyName() == SPELLFAMILY_SHAMAN && classOptions->SpellFamilyFlags & 0x40)
    {
...
bool Unit::IsImmunedToSpell(SpellEntry const* spellInfo)
{
    ...
    for (int i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        if (!spellEffect->EffectMechanic && spellEffect->Effect != SPELL_EFFECT_KNOCK_BACK)
...
bool Unit::IsDamageToThreatSpell(SpellEntry const * spellInfo) const
{
    ...
    {
        case SPELLFAMILY_WARLOCK:
            if (classOptions->SpellFamilyFlags == 0x100) // Searing Pain
                return true;
            break;
        case SPELLFAMILY_SHAMAN:
            if (classOptions->SpellFamilyFlags == SPELLFAMILYFLAG_SHAMAN_FROST_SHOCK)
                return true;
            break;
        case SPELLFAMILY_DEATHKNIGHT:
            if (classOptions->SpellFamilyFlags == 0x20000000) // Rune Strike
                return true;
            break;
    }
...
void Unit::MeleeDamageBonus(Unit *pVictim, uint32 *pdamage, WeaponAttackType attType, SpellEntry const *spellProto)
{
    ...
    for (AuraEffectList::const_iterator i = mModDamagePercentDone.begin(); i != mModDamagePercentDone.end(); ++i)
        ...
        {
            if (!((*i)->GetSpellProto()->AttributesEx5 & SPELL_ATTR_EX5_SPECIAL_ITEM_CLASS_CHECK))
            {
                EquipmentSlots slot;
    ...
    if (spellProto)
        switch(spellProto->GetSpellFamilyName())
        {
            case SPELLFAMILY_DEATHKNIGHT:
                // Glacier Rot
                if (classOptions && classOptions->SpellFamilyFlags & 0x2 || classOptions->SpellFamilyFlags & 0x6)
                    if (AuraEffect * aurEff = GetDummyAuraEffect(SPELLFAMILY_DEATHKNIGHT, 196, 0))
                        if (pVictim->GetDiseasesByCaster(owner->GetGUID()) > 0)
                            AddPctN(DoneTotalMod, aurEff->GetAmount());
            break;
    ...
    if (spellProto)
    {
        ...
        if (spellProto->GetSpellFamilyName() == SPELLFAMILY_DRUID && classOptions->SpellFamilyFlags & 0x00008800)
...
{
    bool targetMustBeDead = spellProto ? bool(spellProto->AttributesEx3 & SPELL_ATTR_EX3_REQUIRE_DEAD_TARGET) : false;
    bool targetCanBeDead = spellProto ? bool(spellProto->AttributesEx2 & SPELL_ATTR_EX2_ALLOW_DEAD_TARGET) : false;
...
int32 Unit::ModSpellDuration(SpellEntry const* spellProto, Unit const* target, int32 duration, bool positive)
{
    ...
    if (target == this)
    {
        ...
        {
            case SPELLFAMILY_DRUID:
                if (classOptions->SpellFamilyFlags & 0x100)
                {
                    // Glyph of Thorns
                ...
                break;
            case SPELLFAMILY_PALADIN:
                if (classOptions->SpellFamilyFlags & 0x00000002)
                {
                    // Glyph of Blessing of Might
                    if (AuraEffect * aurEff = GetAuraEffect(57958, 0))
                        duration += aurEff->GetAmount() * MINUTE * IN_MILLISECONDS;
                }
                else if (classOptions->SpellFamilyFlags & 0x00010000)
                {
...
void Unit::ModSpellCastTime(SpellEntry const* spellProto, int32 & castTime, Spell * spell)
{
    ...
    if (!(spellProto->Attributes & (SPELL_ATTR_UNK4|SPELL_ATTR_TRADESPELL)) && spellProto->GetSpellFamilyName())
...
{
    SpellEntry const *spellProto = aura->GetSpellProto();
    SpellEquippedItemsEntry const* equipItem = spellProto->GetSpellEquippedItems();
    ...
    if (!isVictim && GetTypeId() == TYPEID_PLAYER)
    {
        if (spellProto->GetEquippedItemClass() == ITEM_CLASS_WEAPON)
        {
            ...
            if (!item || item->IsBroken() || item->GetProto()->Class != ITEM_CLASS_WEAPON || !((1<<item->GetProto()->SubClass) & equipItem->EquippedItemSubClassMask))
                return false;
        }
        else if (spellProto->GetEquippedItemClass() == ITEM_CLASS_ARMOR)
        {
            ...
            if (!item || item->IsBroken() || item->GetProto()->Class != ITEM_CLASS_ARMOR || !((1<<item->GetProto()->SubClass) & equipItem->EquippedItemSubClassMask))
                return false;
        }
    }
...
bool Unit::HandleAuraRaidProcFromChargeWithValue(AuraEffect *triggeredByAura)
{
    ...
    if (!(spellProto->GetSpellFamilyName() == SPELLFAMILY_PRIEST && classOpt->SpellFamilyFlags & 0x20))
    {
    ...
    if (jumps > 0)
    {
        ...
        if (spellEffect->EffectRadiusIndex)
            radius = (float)GetSpellRadiusForTarget(triggeredByAura->GetCaster(), sSpellRadiusStore.LookupEntry(spellEffect->EffectRadiusIndex));
        else
...
Aura * Unit::AddAura(uint32 spellId, Unit *target)
{
    ...
    if (!target->isAlive() && !(spellInfo->Attributes & SPELL_ATTR_PASSIVE) && !(spellInfo->AttributesEx2 & SPELL_ATTR_EX2_ALLOW_DEAD_TARGET))
        return NULL;
...
void Unit::ApplyResilience(Unit const* victim, int32* damage) const
{
    ...
    if (source)
        *damage -= target->GetPlayerDamageReduction(*damage);
}
    ...
    for (SpellClickInfoMap::const_iterator itr = clickPair.first; itr != clickPair.second; ++itr)
    {
        if (itr->second.IsFitToRequirements(clicker, this))
        {
            ...
            if (seatId > -1)
            {
                ...
                {
                    SpellEffectEntry const* spellEffect = spellEntry->GetSpellEffect(SpellEffIndex(i));
                    if (!spellEffect)
                        continue;

                    if (spellEffect->EffectApplyAuraName == SPELL_AURA_CONTROL_VEHICLE)
                    {
                        valid = true;
                        break;
                    }
