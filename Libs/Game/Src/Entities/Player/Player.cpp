...
void Player::DeleteFromDB(uint64 playerguid, uint32 accountId, bool updateRealmChars, bool deleteFinally)
{
    ...
    switch (charDelete_method)
    {
        ...
        case CHAR_DELETE_REMOVE:
        {
            ...
            stmt = CharDB.GetPreparedStatement(CHAR_DEL_PLAYER_GM_TICKETS);
            stmt->setUInt32(0, guid);
            trans->Append(stmt);
            ...
            if (CanJoinConstantChannelInZone(channel, current_zone))
            {
                if (!(channel->flags & CHANNEL_DBC_FLAG_GLOBAL))
                {
                    ...
                    if (channel->flags & CHANNEL_DBC_FLAG_CITY_ONLY)
                        currentNameExt = sObjectMgr->GetStringsForDBCLocale(LANG_CHANNEL_CITY);
                    else
...
bool Player::CanCompleteQuest(uint32 quest_id)
{
    if (quest_id)
    {
        ...
        if (q_status.m_status == QUEST_STATUS_INCOMPLETE)
        {
            ...
            if (qInfo->HasFlag(QUEST_STRAWBERRY_FLAGS_DELIVER))
            {
            ...
            if (qInfo->HasFlag(QUEST_STRAWBERRY_FLAGS_KILL_OR_CAST | QUEST_STRAWBERRY_FLAGS_SPEAKTO))
            {
            ...
            if (qInfo->HasFlag(QUEST_STRAWBERRY_FLAGS_EXPLORATION_OR_EVENT) && !q_status.m_explored)
                return false;

            if (qInfo->HasFlag(QUEST_STRAWBERRY_FLAGS_TIMED) && q_status.m_timer == 0)
                return false;
...
bool Player::CanCompleteRepeatableQuest(Quest const *pQuest)
{
    ...
    if (pQuest->HasFlag(QUEST_STRAWBERRY_FLAGS_DELIVER))
        for (uint8 i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; i++)
...
void Player::AddQuest(Quest const *pQuest, Object *questGiver)
{
    ...
    if (pQuest->HasFlag(QUEST_STRAWBERRY_FLAGS_KILL_OR_CAST | QUEST_STRAWBERRY_FLAGS_SPEAKTO))
    {
        for (uint8 i = 0; i < QUEST_OBJECTIVES_COUNT; ++i)
    ...
    uint32 qtime = 0;
    if (pQuest->HasFlag(QUEST_STRAWBERRY_FLAGS_TIMED))
    {
...
{
    if (!m_timedquests.empty() && qInfo->HasFlag(QUEST_STRAWBERRY_FLAGS_TIMED))
    {
        if (msg)
...
void Player::ItemAddedQuestCheck(uint32 entry, uint32 count)
{
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        ...
        if (!qInfo || !qInfo->HasFlag(QUEST_STRAWBERRY_FLAGS_DELIVER))
            continue;
...
void Player::KilledMonsterCredit(uint32 entry, uint64 guid)
{
    ...
    for (uint8 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        ...
        {
            if (qInfo->HasFlag(QUEST_STRAWBERRY_FLAGS_KILL_OR_CAST))
            {
                for (uint8 j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
                {
...
bool Player::LoadFromDB(uint32 guid, SQLQueryHolder *holder)
{
    ...
    //"health, power1, power2, power3, power4, power5, instance_id, speccount, activespec, exploredZones, equipmentCache, knownTitles, actionBars, grantableLevels FROM characters WHERE guid = '%u'", guid);
...
void Player::_LoadInventory(PreparedQueryResult result, uint32 timeDiff)
{
    ...
    if (result)
    {
        ...
        {
            std::string subject = GetSession()->GetStrings(LANG_NOT_EQUIPPED_ITEM);
    ...
    if (result)
    {
        do
        {
            ...
            if (pQuest)
            {
                ...
                if (pQuest->HasFlag(QUEST_STRAWBERRY_FLAGS_TIMED) && !GetQuestRewardStatus(quest_id))
                {
                    AddTimedQuest(quest_id);
...
bool Player::Satisfy(AccessRequirement const* ar, uint32 target_map, bool report)
{
    if (!isGameMaster() && ar)
    {
        ...
        {
            GetSession()->SendAreaTriggerMessage("%s", GetSession()->GetStrings(LANG_INSTANCE_CLOSED));
            return false;
        }
        ...
        if (LevelMin || LevelMax || missingItem || missingQuest || missingAchievement)
        {
            if (report)
            {
                if (missingQuest && !ar->questFailedText.empty())
                    ...
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetStrings(LANG_LEVEL_MINREQUIRED_AND_ITEM), LevelMin, sObjectMgr->GetItemTemplate(missingItem)->Name1.c_str());
                else if (LevelMin)
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetStrings(LANG_LEVEL_MINREQUIRED), LevelMin);
            }
            return false;
        }
    }
