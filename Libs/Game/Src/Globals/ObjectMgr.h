...
#define MIN_STRAWBERRY_STRING_ID           1                    // 'strings'
...
typedef UNORDERED_MAP<int32, StringsLocale> StringsLocaleMap;
...
class ObjectMgr
{
    ...
    public:
        ...
        bool LoadStringss(char const* table, int32 min_value, int32 max_value);
        bool LoadStringss() { return LoadStringss("strings", MIN_STRAWBERRY_STRING_ID, MAX_STRAWBERRY_STRING_ID); }
        ...
        StringsLocale const* GetStringsLocale(int32 entry) const
        {
            StringsLocaleMap::const_iterator itr = mStringsLocaleMap.find(entry);
            if (itr == mStringsLocaleMap.end()) return NULL;
            return &itr->second;
        }
        const char *GetStrings(int32 entry, LocaleConstant locale_idx) const;
        const char *GetStringsForDBCLocale(int32 entry) const { return GetStrings(entry, DBCLocaleIndex); }
...
//        GraveYardMap        mGraveYardMap;
    ...
    private:
        ...
        StringsLocaleMap mStringsLocaleMap;
...
bool LoadStringss(char const* table, int32 start_value = MAX_CREATURE_AI_TEXT_STRING_ID, int32 end_value = std::numeric_limits<int32>::min());
