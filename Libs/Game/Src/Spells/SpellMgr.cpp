...
void SpellMgr::LoadSpellBonusess()
{
    ...
    do
    {
        ...
        {
            const SpellEffectEntry *spellEffect = spell->GetSpellEffect(SpellEffIndex(i));

            if (!spellEffect)
                continue;

            if(spellEffect->EffectBonusCoefficient == sbe.direct_damage || sbe.direct_damage < 0)
                break;

            if(DBCcoef == 0)
                DBCcoef = spellEffect->EffectBonusCoefficient;
        }
        if(DBCcoef > 0)
...
bool SpellMgr::canStackSpellRanks(SpellEntry const *spellInfo)
{
    if (IsPassiveSpell(spellInfo->Id))                       // ranked passive spell
        return false;
    if (spellInfo->powerType != MANA && spellInfo->powerType != POWER_HEALTH)
        return false;
