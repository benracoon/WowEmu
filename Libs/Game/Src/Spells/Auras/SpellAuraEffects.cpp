...
// such checks will speedup strawberry change amount/send for client operations
    ...
    switch(GetAuraType())
    {
        ...
        case SPELL_AURA_SCHOOL_ABSORB:
            ...
            switch(GetSpellProto()->GetSpellFamilyName())
            {
                case SPELLFAMILY_MAGE:
                    // Ice Barrier
                    if (classOptions->SpellFamilyFlags & 0x1 && classOptions->SpellFamilyFlags2 & 0x8)
                    {
                        // +80.68% from sp bonus
                    ...
                    // Fire Ward
                    else if(classOptions->SpellFamilyFlags & 0x8 && classOptions->SpellFamilyFlags2 & 0x8)
                    {
                        // +80.68% from sp bonus
                        DoneActualBenefit += caster->SpellBaseDamageBonus(GetSpellSchoolMask(m_spellProto)) * 0.8068f;
                    }
                    // Frost Ward
                    else if(classOptions->SpellFamilyFlags & 0x100 && classOptions->SpellFamilyFlags2 & 0x8)
                    {
                        // +80.68% from sp bonus
                        DoneActualBenefit += caster->SpellBaseDamageBonus(GetSpellSchoolMask(m_spellProto)) * 0.8068f;
                    }
                ...
                case SPELLFAMILY_WARLOCK:
                    // Shadow Ward
                    if (classOptions->SpellFamilyFlags2 & 0x40)
                    {
                        // +80.68% from sp bonus
                        DoneActualBenefit += caster->SpellBaseDamageBonus(GetSpellSchoolMask(m_spellProto)) * 0.8068f;
                    }
                ...
                case SPELLFAMILY_PRIEST:
                    // Power Word: Shield
                    if (classOptions->SpellFamilyFlags & 0x1 && classOptions->SpellFamilyFlags2 & 0x400)
                    {
                        // +80.68% from sp bonus
                        float bonus = 0.8068f;
        ...
        case SPELL_AURA_MANA_SHIELD:
            ...
            if (!caster)
                break;
            // Mana Shield
            if (GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_MAGE && classOptions->SpellFamilyFlags & 0x8000 && classOptions->SpellFamilyFlags2 & 0x8)
            {
        ...
        case SPELL_AURA_DUMMY:
            if (!caster)
                break;
            // Earth Shield
            if (GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_SHAMAN && classOptions->SpellFamilyFlags & 0x400)
        ...
        case SPELL_AURA_DAMAGE_SHIELD:
            if (!caster)
                break;
            // Thorns
            if (GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_DRUID && classOptions->SpellFamilyFlags & 0x100)
                // 3.3% from sp bonus
        ...
        case SPELL_AURA_PERIODIC_DAMAGE:
            if (!caster)
                break;
            // Rupture
            if (GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_ROGUE && classOptions->SpellFamilyFlags & 0x100000)
            {
                m_canBeRecalculated = false;
            ...
            // Rip
            else if (GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_DRUID && classOptions->SpellFamilyFlags & 0x00800000 && GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
            {
                m_canBeRecalculated = false;
                // 0.01*$AP*cp
                if (caster->GetTypeId() != TYPEID_PLAYER)
            ...
            // Rend
            else if (GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_WARRIOR && classOptions->SpellFamilyFlags & 0x20)
            {
                m_canBeRecalculated = false;
        ...
        case SPELL_AURA_PERIODIC_ENERGIZE:
            if (GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_GENERIC)
            {
                ...
                if (m_spellProto->SpellIconID == 3184 && m_spellProto->SpellVisual[0] == 12495)
                    amount = GetBase()->GetUnitOwner()->GetMaxPower(MANA) * 25 / 10000;
            }
            // Innervate
            else if (m_spellProto->Id == 29166)
                ApplyPctF(amount, float(GetBase()->GetUnitOwner()->GetCreatePowers(MANA)) / GetTotalTicks());
            // Owlkin Frenzy
            else if (m_spellProto->Id == 48391)
                ApplyPctU(amount, GetBase()->GetUnitOwner()->GetCreatePowers(MANA));
            break;
        case SPELL_AURA_PERIODIC_HEAL:
            if (!caster)
                break;
            // Lightwell Renew
            if (GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_PRIEST && classOptions->SpellFamilyFlags2 & 0x4000)
            {
                if (caster->GetTypeId() == TYPEID_PLAYER)
        ...
        case SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN:
            if (!caster)
                break;
            // Icebound Fortitude
            if (GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_DEATHKNIGHT && classOptions->SpellFamilyFlags & 0x00100000)
            {
                if (caster->GetTypeId() == TYPEID_PLAYER)
                {
            ...
            // Hand of Salvation
            else if (GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_PALADIN && classOptions->SpellFamilyFlags & 0x00000100)
            {
                //Glyph of Salvation
        ...
        case SPELL_AURA_MOD_INCREASE_SPEED:
            ...
            if (GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_DRUID && classOptions->SpellFamilyFlags2 & 0x00000008)
...
void AuraEffect::CalculateSpellMod()
{
    switch (GetAuraType())
    {
        ...
        case SPELL_AURA_ADD_PCT_MODIFIER:
            if (!m_spellmod)
            {
                SpellEffectEntry const* spellEffect = GetSpellProto()->GetSpellEffect(SpellEffIndex(EFFECT_0));
                m_spellmod = new SpellModifier(GetBase());
        ...
        case SPELL_AURA_PERIODIC_DUMMY:
            switch(GetSpellProto()->GetSpellFamilyName())
            {
                ...
                case SPELLFAMILY_DEATHKNIGHT:
                    // Chains of Ice
                    if (classOptions && classOptions->SpellFamilyFlags & 0x00004000)
                    {
                        // Get 0 effect aura
                        if (AuraEffect *slow = GetBase()->GetEffect(0))
                        {
...
void AuraEffect::PeriodicTick(AuraApplication * aurApp, Unit * caster) const
{
    ...
    switch(GetAuraType())
    {
        ...
        case SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
        {
            ...
            SpellEffectEntry const* spellEffect = GetSpellProto()->GetSpellEffect(SpellEffIndex(GetEffIndex()));
            ...
            if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
            {
                switch (GetId())
                {
                    ...
                    {
                        uint32 percent =
                            GetEffIndex() < 2 && spellEffect->Effect == SPELL_EFFECT_DUMMY ?
            ...
            if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
            {
                ...
                if (GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_WARLOCK && (classOptions->SpellFamilyFlags & 0x400) && GetSpellProto()->SpellIconID == 544)
                {
                ...
                if (GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_WARLOCK && (classOptions->SpellFamilyFlags & 0x00004000))
                {
        ...
        case SPELL_AURA_PERIODIC_MANA_LEECH:
        {
            ...
            if (m_spellProto->GetSpellFamilyName() == SPELLFAMILY_WARLOCK
                && classOptions->SpellFamilyFlags & 0x00000010)
            {
                int32 manaFeedVal = 0;
    ...
    switch (GetSpellProto()->GetSpellFamilyName())
    {
        ...
        switch (GetId())
        {
            ...
            case 62292: // Blaze (Pool of Tar)
            {
                ...
                SpellEffectEntry const* spellEffect = GetSpellProto()->GetSpellEffect(SpellEffIndex(m_effIndex));
                target->CastSpell((Unit*)NULL, spellEffect->EffectTriggerSpell, true);
                break;
            }
        ...
        case SPELLFAMILY_DRUID:
        {
            switch (GetSpellProto()->Id)
            {
                ...
                case 22842:
                {
                    ...
                    // Should be manauser
                    if (target->getPowerType() != RAGE)
                        break;
                    uint32 rage = target->GetPower(RAGE);
                    // Nothing todo
                    if (rage == 0)
                        break;
            ...
            switch (GetSpellProto()->Id)
            {
                ...
                case 51690:
                {
                    ...
                    UnitList targets;
                    {
                        ...
                        CellPair p(Strawberry::ComputeCellPair(caster->GetPositionX(),caster->GetPositionY()));
                        Cell cell(p);
                        cell.data.Part.reserved = ALL_DISTRICT;

                        Strawberry::AnyUnfriendlyVisibleUnitInObjectRangeCheck u_check(caster, caster, radius);
                        Strawberry::UnitListSearcher<Strawberry::AnyUnfriendlyVisibleUnitInObjectRangeCheck> checker(caster,targets, u_check);

                        TypeContainerVisitor<Strawberry::UnitListSearcher<Strawberry::AnyUnfriendlyVisibleUnitInObjectRangeCheck>, GridTypeMapContainer > grid_object_checker(checker);
                        TypeContainerVisitor<Strawberry::UnitListSearcher<Strawberry::AnyUnfriendlyVisibleUnitInObjectRangeCheck>, WorldTypeMapContainer > world_object_checker(checker);
            ...
            case SPELLFAMILY_GENERIC:
            {
                switch (auraId)
                {
                    ...
                    case 23493:
                    {
                        ...
                        int32 mana = caster->GetMaxPower(MANA);
                        if (mana)
                        {
                            mana /= 10;
                            caster->EnergizeBySpell(caster, 23493, mana, MANA);
                        }
                        return;
                    }
                    // Nitrous Boost
                    case 27746:
                        if (target->GetPower(MANA) >= 10)
                        {
                            target->ModifyPower(MANA, -10);
                            target->SendEnergizeSpellLog(caster, 27746, 10, MANA);
                        }
                        else
                    ...
                    // Detonate Mana
                    case 27819:
                        if (int32 mana = (int32)(target->GetMaxPower(MANA) / 10))
                        {
                            mana = target->ModifyPower(MANA, -mana);
...
bool AuraEffect::IsAffectedOnSpell(SpellEntry const *spell) const
{
    ...
    if (!spell || !spellEffect)
        return false;
    // Check family name
    if (classOpt->SpellFamilyName != classOpt->SpellFamilyName)
        return false;

    // Check EffectClassMask
    if (spellEffect->EffectSpellClassMaskA[m_effIndex] & classOpt->SpellFamilyFlags)
        return true;
    return false;
}
...
{
    SpellEffectEntry const* spellEffect = m_spellProto->GetSpellEffect(SpellEffIndex(GetEffIndex()));
    if (!spellEffect)
        return;
    ...
    if (spellEffect->EffectApplyAuraName == SPELL_AURA_PERIODIC_TRIGGER_SPELL &&
            uint32(GetSpellDuration(m_spellProto)) == spellEffect->EffectAmplitude)
        return;
...
void AuraEffect::HandleAuraModShapeshift(AuraApplication const * aurApp, uint8 mode, bool apply) const
{
            ...
            switch (form)
            {
                ...
                case FORM_DIREBEAR:
                {
                    ...
                    switch (GetMiscValue())
                    {
                        ...
                        default:
                        {
                            uint32 newEnergy = std::min(target->GetPower(ENERGY), FurorChance);
                            target->SetPower(ENERGY, newEnergy);
                        }
                        break;
                    }
                    break;
    ...
    else
    {
        ...
        switch (form)
        {
            ...
            case FORM_BERSERKERSTANCE:
            {
                ...
                if (target->GetPower(RAGE) > Rage_val)
                    target->SetPower(RAGE,Rage_val);
                break;
            }
            default:
                break;
    ...
    if (target->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        for (uint8 i = 0; i < MAX_SHAPESHIFT_SPELLS; ++i)
        {
            ...
            if (!shapeInfo->Stances)
                continue;

            if (apply)
                target->ToPlayer()->AddTemporarySpell(shapeInfo->Stances);
            else
                target->ToPlayer()->RemoveTemporarySpell(shapeInfo->Stances);
        }
    }
}
...
void AuraEffect::HandleAuraMounted(AuraApplication const * aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        {
            SpellEffectEntry const* spellEffect = GetSpellProto()->GetSpellEffect(SpellEffIndex(i));
            if (!spellEffect)
                continue;

            if (spellEffect->Effect == SPELL_EFFECT_SUMMON
                && spellEffect->EffectMiscValue == GetMiscValue())
                display_id = 0;
        }
...
{
    SpellClassOptionsEntry const* classOptions = GetSpellProto()->GetSpellClassOptions();
    SpellEffectEntry const* spellEffect = m_spellProto->GetSpellEffect(SpellEffIndex(EFFECT_0));
    if (!(mode & AURA_EFFECT_HANDLE_CHANGE_AMOUNT_MASK))
        return;
    ...
    if (mode & AURA_EFFECT_HANDLE_REAL)
    {
        ...
        if (apply)
        {
            ...
            if (caster && m_spellProto->GetSpellFamilyName() == SPELLFAMILY_WARRIOR &&
                classOptions->SpellFamilyFlags & 0x4)
            {
        ...
        else
        {
            ...
            switch(m_spellProto->GetSpellFamilyName())
            {
                ...
                case SPELLFAMILY_PRIEST:
                    // Vampiric Touch
                    if (classOptions->SpellFamilyFlags & 0x0400 && aurApp->GetRemoveMode() == AURA_REMOVE_BY_ENEMY_SPELL)
                    {
                        if (AuraEffect const * aurEff = GetBase()->GetEffect(1))
                        {
                ...
                case SPELLFAMILY_DEATHKNIGHT:
                    ...
                    if (GetId() == 61777)
                        target->CastSpell(target, spellEffect->EffectTriggerSpell, true);
                    break;
                default:
                    break;
            }
...
void AuraEffect::HandleChannelDeathItem(AuraApplication const * aurApp, uint8 mode, bool apply) const
{
    ...
    if (!(apply))
    {
        ...
        // Soul Shard
        if (spellEffect->EffectItemType == 6265)
        {
        ...
        uint8 msg = plCaster->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, spellEffect->EffectItemType, count, &noSpaceForCount);
        if (msg != EQUIP_ERR_OK)
        {
            count-=noSpaceForCount;
            plCaster->SendEquipError(msg, NULL, NULL, spellEffect->EffectItemType);
            if (count == 0)
                return;
        }

        Item* newitem = plCaster->StoreNewItem(dest, spellEffect->EffectItemType, true);
        if (!newitem)
        {
...
void AuraEffect::HandleAuraLinked(AuraApplication const * aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        if (GetAmount())
            caster->CastCustomSpell(target, spellEffect->EffectTriggerSpell, &m_amount, NULL, NULL, true, NULL, this);
        else
            caster->CastSpell(target, spellEffect->EffectTriggerSpell,true, NULL, this);
    }
    else
        target->RemoveAura(spellEffect->EffectTriggerSpell, GetCasterGUID(), 0, AuraRemoveMode(aurApp->GetRemoveMode()));
}
