...
void Map::MoveAllCreaturesInMoveList()
{
    while (!i_creaturesToMove.empty())
    {
        ...
        if (CreatureCellRelocation(c, Cell(Strawberry::ComputeCellPair(cm.x, cm.y))))
        {
            // update pos
...
void Map::SendInitTransports(Player* player)
{
    ...
    transData.m_map = player->GetMapId();
...
void InstanceMap::CreateInstanceData(bool load)
{
    ...
    if (load)
    {
        ...
        QueryResult result = CharDB.PQuery("SELECT data, completedEncounters FROM instance WHERE map = '%u' AND id = '%u'", GetId(), i_InstanceId);
        if (result)
        {
