...
namespace MMAP
{
    ...
    {
        return sWorld->getBoolConfig(CONFIG_BOOL_MMAP_ENABLED)
    ...
    bool MMapManager::loadMapData(uint32 mapId)
    {
        ...
        if (!file)
        {
            sLog->outDebug(LOG_FILTER_PATHFINDING, "MMAP:loadMapData: Error: Could not open mmap file '%s'", fileName);
            delete [] fileName;
            return false;
        }
    ...
    bool MMapManager::loadMap(uint32 mapId, int32 x, int32 y)
    {
        ...
        if (!file)
        {
            sLog->outDebug(LOG_FILTER_PATHFINDING, "MMAP:loadMap: Could not open mmtile file '%s'", fileName);
            delete [] fileName;
            return false;
        }
        delete [] fileName;
    ...
    bool MMapManager::unloadMap(uint32 mapId, int32 x, int32 y)
    {
        ...
        if (loadedMMaps.find(mapId) == loadedMMaps.end())
        {
            ...
            sLog->outDebug(LOG_FILTER_PATHFINDING, "MMAP:unloadMap: Asked to unload not loaded navmesh map. %03u%02i%02i.mmtile", mapId, x, y);
            return false;
        }
        ...
        {
            // file may not exist, therefore not loaded
            sLog->outDebug(LOG_FILTER_PATHFINDING, "MMAP:unloadMap: Asked to unload not loaded navmesh tile. %03u%02i%02i.mmtile", mapId, x, y);
            return false;
        }
    ...
    bool MMapManager::unloadMap(uint32 mapId)
    {
        ...
        {
            // file may not exist, therefore not loaded
            sLog->outDebug(LOG_FILTER_PATHFINDING, "MMAP:unloadMap: Asked to unload not loaded navmesh map %03u", mapId);
            return false;
        }
