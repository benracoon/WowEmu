...
bool ArenaTeam::Create(uint32 captainGuid, uint8 type, std::string teamName, uint32 backgroundColor, uint8 emblemStyle, uint32 emblemColor, uint8 borderStyle, uint32 borderColor)
{
    ...
    PreparedStatement* stmt = CharDB.GetPreparedStatement(CHAR_ADD_ARENA_TEAM);
...
bool ArenaTeam::AddMember(const uint64 playerGuid)
{
    ...
    else
    {
        ...
        PreparedStatement* stmt = CharDB.GetPreparedStatement(CHAR_LOAD_PLAYER_NAME_CLASS);
    ...
    PreparedStatement* stmt = CharDB.GetPreparedStatement(CHAR_LOAD_MATCH_MAKER_RATING);
    stmt->setUInt32(0, GUID_LOPART(playerGuid));
    ...
    stmt = CharDB.GetPreparedStatement(CHAR_SET_ARENA_TEAM_MEMBER);
    stmt->setUInt32(0, TeamId);
...
void ArenaTeam::SetCaptain(const uint64 guid)
{
    ...
    // Update database
    PreparedStatement* stmt = CharDB.GetPreparedStatement(CHAR_UPDATE_ARENA_TEAM_CAPTAIN);
...
void ArenaTeam::DelMember(uint64 guid, bool cleanDb)
{
    ...
    if (cleanDb)
    {
        PreparedStatement* stmt = CharDB.GetPreparedStatement(CHAR_DEL_ARENA_TEAM_MEMBER);
...
void ArenaTeam::Disband(WorldSession* session)
{
    ...
    PreparedStatement* stmt = CharDB.GetPreparedStatement(CHAR_DEL_ARENA_TEAM);
    stmt->setUInt32(0, TeamId);
    trans->Append(stmt);

    stmt = CharDB.GetPreparedStatement(CHAR_DEL_ARENA_TEAM_MEMBERS);
    stmt->setUInt32(0, TeamId);
    trans->Append(stmt);
...
void ArenaTeam::SaveToDB()
{
    ...
    PreparedStatement* stmt = CharDB.GetPreparedStatement(CHAR_UPDATE_ARENA_TEAM_STATS);
    ...
    for (MemberList::const_iterator itr = Members.begin(); itr !=  Members.end(); ++itr)
    {
        stmt = CharDB.GetPreparedStatement(CHAR_UPDATE_ARENA_TEAM_MEMBER);
        ...
        stmt = CharDB.GetPreparedStatement(CHAR_UPDATE_CHARACTER_ARENA_STATS);
