...
template<class T>
void WaypointMovementGenerator<T>::Interrupt(T &owner)
{
    owner.ClearUnitState(UNIT_STAT_ROAMING | UNIT_STAT_DISTRACTED);
}
...
template void WaypointMovementGenerator<Creature>::Interrupt(Creature &);
template void WaypointMovementGenerator<Player>::Interrupt(Player &);
...
void WaypointMovementGenerator<Creature>::MoveToNextNode(CreatureTraveller &traveller)
{
    ...
    PathFinder sub_path(owner, node->x, node->y, node->z);
...
void FlightPathMovementGenerator::Initialize(Player &player)
{
    ...
    player.SendMonsterMoveByPath(path, GetCurrentNode(), pathEndPoint, player.GetUnitMovementFlags(), traveltime );
}
...
{
    player.ClearUnitState(UNIT_STAT_FLEEING);
}
