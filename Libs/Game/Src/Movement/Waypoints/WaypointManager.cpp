...
void WaypointMgr::Load()
{
    ...
    QueryResult result = WorldDB.Query("SELECT id, point, position_x, position_y, position_z, move_flag, delay, action, action_chance FROM waypoint_data ORDER BY id, point");
...
void WaypointMgr::ReloadPath(uint32 id)
{
    ...
    QueryResult result = WorldDB.PQuery("SELECT point, position_x, position_y, position_z, move_flag, delay, action, action_chance FROM waypoint_data WHERE id = %u ORDER BY point", id);
    if (!result)
        return;
