...
{
    public:
        PathFinder(Unit const* owner, float destX, float destY, float destZ, bool useStraightPath = false, bool forceDest = false);
        ~PathFinder();
    ...
    private:
        ...
        bool getSteerTarget(const float* startPos, const float* endPos, float minTargetDist, const dtPolyRef* path, uint32 pathSize, float* steerPos,
                            unsigned char& steerPosFlag, dtPolyRef& steerPosRef);
        dtStatus findSmoothPath(const float* startPos, const float* endPos, const dtPolyRef* polyPath, uint32 polyPathSize,
                                float* smoothPath, int* smoothPathSize, bool &usedOffmesh, uint32 smoothPathMaxSize);
};
