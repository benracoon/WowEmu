...
////////////////// PathFinder //////////////////
PathFinder::PathFinder(const Unit* owner, float destX, float destY, float destZ,
                   bool useStraightPath, bool forceDest) :
    m_polyLength(0), m_type(PATHFIND_BLANK),
    m_useStraightPath(useStraightPath), m_forceDestination(forceDest),  m_pointPathLimit(MAX_POINT_PATH_LENGTH),
    m_sourceUnit(owner), m_navMesh(NULL), m_navMeshQuery(NULL)
{
    ...
    sLog->outDebug(LOG_FILTER_PATHFINDING, "++ PathFinder::PathFinder for %u \n", m_sourceUnit->GetGUIDLow());
    ...
    if (m_navMesh && m_navMeshQuery && HaveTile(endPoint) && !m_sourceUnit->HasUnitState(UNIT_STAT_IGNORE_PATHFINDING))
...
bool PathFinder::Update(float destX, float destY, float destZ, bool forceDest)
{
    ...
    sLog->outDebug(LOG_FILTER_PATHFINDING, "++ PathFinder::Update() for %u \n", m_sourceUnit->GetGUIDLow());
    ...
    if (inRange(oldDest, newDest, dist, dist) && m_pathPoints.size() > 2)
    {
        ...
        sLog->outDebug(LOG_FILTER_PATHFINDING, "++ PathFinder::Update:: precalculated path\n");
...
void PathFinder::BuildPolyPath(const PathNode &startPos, const PathNode &endPos)
{
...
bool PathFinder::HaveTile(const PathNode &p) const
{
    int tx, ty;
...
bool PathFinder::inRange(const PathNode &p1, const PathNode &p2, float r, float h) const
{
...
float PathFinder::dist3DSqr(const PathNode &p1, const PathNode &p2) const
{
    const float dx = p2.x - p1.x;
    const float dy = p2.y - p1.y;
    const float dz = p2.z - p1.z;
