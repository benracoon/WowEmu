...
bool ChatLink::ValidateName(DBCString buffer, const char* /*context*/)
{
    _name = buffer;
    return true;
}
...
bool ItemChatLink::ValidateName(DBCString buffer, const char* context)
{
...
bool QuestChatLink::ValidateName(DBCString buffer, const char* context)
{
    ChatLink::ValidateName(buffer, context);
...
bool SpellChatLink::ValidateName(DBCString buffer, const char* context)
{
    ChatLink::ValidateName(buffer, context);
    ...
    if (_spell->Attributes & SPELL_ATTR0_TRADESPELL)
    {
        ...
        for (uint8 i = 0; i < TOTAL_LOCALES; ++i)
        {
            ...
            if (skillLineNameLength > 0 && strncmp(skillLine->name[i], buf, skillLineNameLength) == 0)
            {
                ...
                // -2 = strlen(": ")
                uint32 spellNameLength = strlen(buf) - skillLineNameLength - 2;
                memcpy(buf, buf + skillLineNameLength + 2, spellNameLength + 1);
            }
        }
    }
...
bool AchievementChatLink::ValidateName(DBCString buffer, const char* context)
{
    ChatLink::ValidateName(buffer, context);
