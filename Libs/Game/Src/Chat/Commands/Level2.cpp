...
bool ChatHandler::HandleMuteCommand(const char* args)
{
    ...
    if (target)
    {
        ...
        RealmDB.PExecute("UPDATE account SET mutetime = " SI64FMTD " WHERE id = '%u'", mutetime, account_id);
    ...
    else
    {
        ...
        RealmDB.PExecute("UPDATE account SET mutetime = %d WHERE id = %u", muteTime, account_id);
    }
...
bool ChatHandler::HandleUnmuteCommand(const char* args)
{
    ...
    RealmDB.PExecute("UPDATE account SET mutetime = '0' WHERE id = '%u'", account_id);
...
bool ChatHandler::HandlePInfoCommand(const char* args)
{
    ...
    else
    {
        ...
        //                                         0          1      2      3        4     5      6    7
        QueryResult result = CharDB.PQuery("SELECT totaltime, level, money, account, race, class, map, zone FROM characters "
    ...
    std::string username = GetStrings(LANG_ERROR);
    std::string email = GetStrings(LANG_ERROR);
    std::string last_ip = GetStrings(LANG_ERROR);
    uint32 security = 0;
    std::string last_login = GetStrings(LANG_ERROR);

    QueryResult result = RealmDB.PQuery("SELECT a.username, aa.gmlevel, a.email, a.last_ip, a.last_login, a.mutetime "
                                                "FROM account a "
    ...
    PSendSysMessage(LANG_PINFO_ACCOUNT, (target?"":GetStrings(LANG_OFFLINE)), nameLink.c_str(), GUID_LOPART(target_guid), username.c_str(), accId, email.c_str(), security, last_ip.c_str(), last_login.c_str(), latency);

    std::string bannedby = "unknown";
    std::string banreason = "";
    if (QueryResult result = RealmDB.PQuery("SELECT unbandate, bandate = unbandate, bannedby, banreason FROM account_banned "
    ...
    else if (QueryResult result = CharDB.PQuery("SELECT unbandate, bandate = unbandate, bannedby, banreason FROM character_banned "
    ...
    if (target)
    {
        if (AreaTableEntry const* zone = GetAreaEntryByAreaID(area->zone))
            PSendSysMessage(LANG_PINFO_MAP_ONLINE, map->name, zone->area_name, area->area_name, phase);
        else
            PSendSysMessage(LANG_PINFO_MAP_ONLINE, map->name, area->area_name, "--", phase);
    }
    else
        PSendSysMessage(LANG_PINFO_MAP_OFFLINE, map->name, area->area_name);
...
bool ChatHandler::HandleCharacterRenameCommand(const char* args)
{
    ...
    else
    {
        ...
        CharDB.PExecute("UPDATE characters SET at_login = at_login | '1' WHERE guid = '%u'", GUID_LOPART(target_guid));
    }
...
bool ChatHandler::HandleCharacterCustomizeCommand(const char* args)
{
    ...
    if (target)
    {
        ...
        CharDB.PExecute("UPDATE characters SET at_login = at_login | '8' WHERE guid = '%u'", target->GetGUIDLow());
    }
    else
    {
        std::string oldNameLink = playerLink(target_name);
        ...
        CharDB.PExecute("UPDATE characters SET at_login = at_login | '8' WHERE guid = '%u'", GUID_LOPART(target_guid));
    }
...
bool ChatHandler::HandleCharacterChangeFactionCommand(const char * args)
{
    ...
    if (target)
    {
        ...
        CharDB.PExecute("UPDATE characters SET at_login = at_login | '64' WHERE guid = %u", target->GetGUIDLow());
    }
    else
    {
        std::string oldNameLink = playerLink(target_name);
        ...
        // TODO : add text into database
        PSendSysMessage(LANG_CUSTOMIZE_PLAYER_GUID, oldNameLink.c_str(), GUID_LOPART(target_guid));
        CharDB.PExecute("UPDATE characters SET at_login = at_login | '64' WHERE guid = %u", GUID_LOPART(target_guid));
    }
...
bool ChatHandler::HandleCharacterChangeRaceCommand(const char * args)
{
    ...
    if (target)
    {
        ...
        CharDB.PExecute("UPDATE characters SET at_login = at_login | '128' WHERE guid = %u", target->GetGUIDLow());
    }
    else
    {
        std::string oldNameLink = playerLink(target_name);
        ...
        // TODO : add text into database
        PSendSysMessage(LANG_CUSTOMIZE_PLAYER_GUID, oldNameLink.c_str(), GUID_LOPART(target_guid));
        CharDB.PExecute("UPDATE characters SET at_login = at_login | '128' WHERE guid = %u", GUID_LOPART(target_guid));
    }
...
bool ChatHandler::HandleCharacterReputationCommand(const char* args)
{
    ...
    for (FactionStateList::const_iterator itr = targetFSL.begin(); itr != targetFSL.end(); ++itr)
    {
        ...
        std::string rankName = GetStrings(ReputationRankStrIndex[rank]);
...
bool ChatHandler::HandleLookupEventCommand(const char* args)
{
    ...
    for (uint32 id = 0; id < events.size(); ++id)
    {
        ...
        if (Utf8FitTo(descr, wnamepart))
        {
            ...
            char const* active = activeEvents.find(id) != activeEvents.end() ? GetStrings(LANG_ACTIVE) : "";
...
bool ChatHandler::HandleLookupPlayerIpCommand(const char* args)
{
    ...
    QueryResult result = RealmDB.PQuery ("SELECT id, username FROM account WHERE last_ip = '%s'", ip.c_str ());
...
bool ChatHandler::HandleLookupPlayerAccountCommand(const char* args)
{
    ...
    RealmDB.EscapeString (account);

    QueryResult result = RealmDB.PQuery ("SELECT id, username FROM account WHERE username = '%s'", account.c_str ());
...
bool ChatHandler::HandleLookupPlayerEmailCommand(const char* args)
{
    ...
    RealmDB.EscapeString (email);

    QueryResult result = RealmDB.PQuery ("SELECT id, username FROM account WHERE email = '%s'", email.c_str ());
...
bool ChatHandler::LookupPlayerSearchCommand(QueryResult result, int32 limit)
{
    ...
    do
    {
        ...
        QueryResult chars = CharDB.PQuery("SELECT guid, name FROM characters WHERE account = '%u'", acc_id);
        if (chars)
        {
    ...
    for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); id++)
    {
        ...
        if (titleInfo)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                char const* knownStr = target && target->HasTitle(titleInfo) ? GetStrings(LANG_KNOWN) : "";

                char const* activeStr = target && target->GetUInt32Value(PLAYER_CHOSEN_TITLE) == titleInfo->bit_index
                    ? GetStrings(LANG_ACTIVE)
                    : "";
...
bool ChatHandler::HandleCharacterTitlesCommand(const char* args)
{
    ...
    char const* knownStr = GetStrings(LANG_KNOWN);
