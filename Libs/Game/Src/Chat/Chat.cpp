...
{
    const char *format = GetStrings(entry);
    char str[1024];
    va_list ap;
    va_start(ap, entry);
...
const char* ChatHandler::GetStrings(int32 entry) const
{
    return m_session->GetStrings(entry);
}
...
{
    SendSysMessage(GetStrings(entry));
}
...
const char *CliHandler::GetStrings(int32 entry) const
{
...
{
    return GetStrings(LANG_CONSOLE_COMMAND);
}
