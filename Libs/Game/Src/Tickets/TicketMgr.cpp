...
{
    uint8 index = 0;
    PreparedStatement* stmt = CharDB.GetPreparedStatement(CHAR_ADD_GM_TICKET);
...
void TicketMgr::LoadTickets()
{
    ...
    PreparedStatement* stmt = CharDB.GetPreparedStatement(CHAR_LOAD_GM_TICKETS);
...
void TicketMgr::LoadSurveys()
{
    ...
    uint32 oldMSTime = getMSTime();
    if (QueryResult result = CharDB.Query("SELECT MAX(surveyId) FROM gm_surveys"))
