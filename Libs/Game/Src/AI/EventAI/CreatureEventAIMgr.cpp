...
void CreatureEventAIMgr::LoadCreatureEventAI_Texts()
{
    ...
    // Load EventAI Text
    sObjectMgr->LoadStringss("creature_ai_texts", MIN_CREATURE_AI_TEXT_STRING_ID, MAX_CREATURE_AI_TEXT_STRING_ID);

    // Gather Additional data from EventAI Texts
    QueryResult result = WorldDB.Query("SELECT entry, sound, type, language, emote FROM creature_ai_texts");
    ...
    do
    {
        ...
        if (!sObjectMgr->GetStringsLocale(i))
        {
...
void CreatureEventAIMgr::LoadCreatureEventAI_Summons()
{
    ...
    QueryResult result = WorldDB.Query("SELECT id, position_x, position_y, position_z, orientation, spawntimesecs FROM creature_ai_summons");
...
void CreatureEventAIMgr::LoadCreatureEventAI_Scripts()
{
    ...
    // Gather event data
    QueryResult result = WorldDB.Query("SELECT id, creature_id, event_type, event_inverse_phase_mask, event_chance, event_flags, "
        ...
        for (uint32 j = 0; j < MAX_ACTIONS; j++)
        {
            ...
            switch (action.type)
            {
                ...
                case ACTION_T_QUEST_EVENT:
                    ...
                    {
                        if (!qid->HasFlag(QUEST_STRAWBERRY_FLAGS_EXPLORATION_OR_EVENT))
